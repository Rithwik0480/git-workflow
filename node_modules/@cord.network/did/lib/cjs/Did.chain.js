...
exports.didSignatureToChain = exports.generateDidAuthenticatedTx = exports.getStoreTx = exports.publicKeyToChain = exports.serviceFromChain = exports.serviceToChain = exports.validateService = exports.documentFromChain = exports.fromChain = exports.resourceIdToChain = exports.toChain = void 0;
...
function fromChain(encoded) {
    return (0, Did_utils_js_1.getDidUri)(utils_1.Crypto.encodeAddress(encoded, utils_1.ss58Format));
}
...
function documentFromChain(encoded) {
    const { publicKeys, authenticationKey, assertionKey, delegationKey, keyAgreementKeys, lastTxCounter, } = encoded.unwrap();
...
function validateService(endpoint) {
    ...
    if (id.startsWith('did:cord')) {
        throw new utils_1.SDKErrors.DidError(`This function requires only the URI fragment part (following '#') of the service ID, not the DID URI, which is violated by id "${id}"`);
    }
 ...
 */
async function getStoreTx(input, sign) {
    ...
    if (assertionMethod && assertionMethod.length > 1) {
        throw new utils_1.SDKErrors.DidError(`More than one assertion key (${assertionMethod.length}) specified. The chain can only store one.`);
    }
    ...
    const newAssertionKey = assertionMethod &&
