...
import { getAddressByKey, getDidUri, parse, } from './Did.utils.js';
...
export function validateService(endpoint) {
    ...
    if (id.startsWith('did:cord')) {
        throw new SDKErrors.DidError(`This function requires only the URI fragment part (following '#') of the service ID, not the DID URI, which is violated by id "${id}"`);
    }
 ...
 */
export async function getStoreTx(input, sign) {
    ...
    if (assertionMethod && assertionMethod.length > 1) {
        throw new SDKErrors.DidError(`More than one assertion key (${assertionMethod.length}) specified. The chain can only store one.`);
    }
