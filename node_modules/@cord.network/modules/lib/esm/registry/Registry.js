 ...
 * @preferred
 */
import { Identifier, Crypto, SDKErrors, jsonabc, DataUtils, DecoderUtils, } from '@cord.network/utils';
import { REGISTRY_IDENT, REGISTRY_PREFIX, AUTHORIZATION_IDENT, AUTHORIZATION_PREFIX, } from '@cord.network/types';
 ...
 */
export function getUriForRegistry(serializedRegistry, creator) {
    ...
    const scaleEncodedCreator = api
        ...
        .toU8a();
    const digest = blake2AsHex(Uint8Array.from([...scaleEncodedRegistry, ...scaleEncodedCreator]));
...
export function getAuthorizationIdentifier(registry, authority, creator) {
    const api = ConfigService.get('api');
    const scaleEncodedRegistry = api
    ...
    const scaleEncodedCreator = api
        .createType('AccountId', Did.toChain(creator))
        .toU8a();
    const digest = blake2AsHex(Uint8Array.from([
        ...scaleEncodedRegistry,
        ...scaleEncodedAuthority,
        ...scaleEncodedCreator,
    ]));
    const authorizationId = Identifier.hashToUri(digest, AUTHORIZATION_IDENT, AUTHORIZATION_PREFIX);
...
export async function verifyAuthorization(auth) {
    ...
    if (encoded.isNone)
        throw new SDKErrors.AuthorizationIdMissingError(`Authorization with identifier ${identifier} is not registered on chain`);
}
/**
 * Checks on the CORD blockchain whether a Registry is anchored.
 *
 ...
 */
export async function fetchAuthorizationDetailsfromChain(auth) {
    const api = ConfigService.get('api');
    ...
    if (registryAuthoriation.isNone) {
        throw new SDKErrors.AuthorizationIdentifierMissingError(`Registry Authorization with identifier ${authorizationId} is not registered on chain`);
    }
    else {
        return registryAuthoriation;
    }
}
export function getAuthorizationDetails(encodedEntry) {
...
export function uriToIdentifier(registryId) {
